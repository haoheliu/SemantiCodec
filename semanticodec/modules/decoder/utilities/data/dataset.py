...
class AudioDataset(Dataset):
    ...
    def random_segment_wav(self, waveform, target_length):
        ...
        for i in range(10):
            ...
            if(torch.max(torch.abs(waveform[:, random_start : random_start + target_length])>1e-4)):
                break
    ...
    def mel_spectrogram_train(self, y):
        ...
        if self.mel_fmax not in self.mel_basis:
            mel = librosa_mel_fn(self.sampling_rate, self.filter_length, self.n_mel, self.mel_fmin, self.mel_fmax)
        ...
        stft_spec = torch.stft(y, self.filter_length, hop_length=self.hop_length, win_length=self.win_length, window=self.hann_window[str(y.device)],
...
if __name__ == "__main__":
    ...
    config = yaml.load(
        open(
            "/mnt/bn/lqhaoheliu/project/audio_generation_diffusion/config/37_final_codec/2024_03_13_v.yaml",
            "r",
        ),
